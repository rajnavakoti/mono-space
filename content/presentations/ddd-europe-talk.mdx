...
<CompareTable headers="Service|They call them...|ID field" rows="Shipment|buyer|buyerId||Consignee|customer|customerId||Inventory|user|userId||Carrier|recipient|recipientId||Invoicing|account|accountId||Tracking|user|userId" />

> *"Six services. Six names. Same person. The translation map between contexts is missing."*

<Notes>
"First check: what does each service call the person receiving the delivery? In a language, this is vocabulary. In DDD, this is the ubiquitous language. Shipment calls them 'buyer.' Consignee — 'customer.' Inventory — 'user.' Carrier — 'recipient.' Invoicing — 'account.' Tracking — 'user' again. Same fields. Different words. This isn't bounded context language. It's drift. The translation map is missing."
</Notes>
